        ...
        for (Set<Integer> cluster : connectedComponents) {
            ...
            for (int attributeId : cluster) {
                if (attributeId < d1Attributes) {
                    ...
                    System.out.println(attributeModels1[attributeId].getInstanceName() +"   "+ clusterId);
                } else {
                    attributeClusters[1].put(attributeModels2[attributeId - d1Attributes].getInstanceName(), clusterId);
                    System.out.println(attributeModels2[attributeId - d1Attributes].getInstanceName()+"   "+ clusterId);
                }
            }
        }
    }
...
//            System.out.println("");
        ...
        for (int i = 0; i < totalAttributes; i++) {
            if (MINIMUM_ATTRIBUTE_SIMILARITY_THRESHOLD < maxSimillarity[i]) {
                namesGraph.addEdge(i, mostSimilarName[i]);
                System.out.println(i +"  "+ mostSimilarName[i] + " " + d1Attributes);
                try{
                   // System.out.println( attributeModels1[i].getInstanceName() +"  conectado a " + attributeModels2[mostSimilarName[i]- d1Attributes].getInstanceName() );
                	
                	if (i < d1Attributes) {
                		System.out.println( attributeModels1[i].getInstanceName() +"  conectado a " + attributeModels2[mostSimilarName[i]- d1Attributes].getInstanceName() );
                    } else {
                    	System.out.println( attributeModels1[i- d1Attributes].getInstanceName() +"  conectado a " + attributeModels2[mostSimilarName[i]].getInstanceName() );
                    }
                	
                }catch(Exception e){
                	System.out.println(mostSimilarName[i]- d1Attributes);
                }
            }
        }
        return namesGraph;
